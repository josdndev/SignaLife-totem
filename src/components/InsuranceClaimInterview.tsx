...
import { Send, User, Shield, ChevronRight, Mic, MicOff, Keyboard as KeyboardIcon } from 'lucide-react';
...
export function InsuranceClaimInterview({ onComplete }: InterviewProps) {
  ...
  const requestInsuranceResponse = async (history: ChatMessage[]) => {
    setIsLoading(true);
    try {
      const response = await fetch('/api/insurance-interview', {
        method: 'POST',
      ...
      if (response.ok) {
        ...
        if (replyTxt.includes("INSURANCE_COLLECTED")) {
          replyTxt = replyTxt.replace(/INSURANCE_COLLECTED\.?/g, '').trim();
          setIsDone(true);
          if (replyTxt) {
              setMessages(prev => [...prev, { role: 'model', content: replyTxt }]);
              speak(replyTxt);
          } else {
              speak("Información recopilada, procesando declaración.");
          }
        } else {
          setMessages(prev => [...prev, { role: 'model', content: replyTxt }]);
          speak(replyTxt);
  ...
  const handleSend = () => {
    ...
    requestInsuranceResponse(updatedHistory);
  };

  const finishInterview = () => {
    onComplete(messages.filter(m => m.role === 'user').map(m => m.content).join("\n"));
  };
  ...
  return (
    <div className="flex flex-col h-full bg-slate-50 min-h-[500px]">
      <div className="p-4 md:p-8 bg-emerald-600 text-white flex flex-col items-center">
        <h2 className="text-xl md:text-2xl font-semibold tracking-tight">Declaración de Siniestro</h2>
        <p className="text-emerald-100 mt-2 font-mono text-sm max-w-sm text-center">
          Responde las preguntas para completar tu carta de reclamo.
        </p>
      </div>
      ...
      <div className="p-4 bg-white border-t border-slate-200">
        <AnimatePresence mode="wait">
          {isDone ? (
            ...
            >
              <p className="text-sm text-slate-500 mb-3 text-center">Información recopilada completa.</p>
              <button 
